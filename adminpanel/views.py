...
from adminpanel.forms import ProductForm, VariationForm, CatagoryForm
...
def manage_variation(request):
  if request.user.is_admin:
    if request.method == 'POST':
      ...
      variations = Variation.objects.filter(Q(product__product_name__icontains=keyword) | Q(variation_catagory__icontains=keyword) | Q(variation_value__icontains=keyword)).order_by('id')
...
def manage_product(request):
  if request.user.is_admin:
    if request.method == 'POST':
      keyword = request.POST['keyword']
      products = Products.objects.filter(Q(product_name__icontains=keyword) | Q(slug__icontains=keyword) | Q(catagory__catagory_name__icontains=keyword)).order_by('id')
...
def manage_category(request):
  if request.user.is_admin:
    if request.method == 'POST':
      keyword = request.POST['keyword']
      categories = Catagory.objects.filter(Q(catagory_name__icontains=keyword) | Q(slug__icontains=keyword)).order_by('id')
